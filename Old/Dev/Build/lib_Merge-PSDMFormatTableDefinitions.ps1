Function Merge-PSDMFormatTableDefinitions
{
    ...
    param(       
        ...
        [Parameter(
            ParameterSetName='PSXMLString',
            ValueFromPipeline = $true)
        ]
        [PSModuleDevelopment.Format.TableDefinition]$PS1XML,
...
'@
    ...
    Process{
        ...
        elseif($PSCmdlet.ParameterSetName -eq "PSXMLString") {
            $thisObject = @{
                xml = ""
                Path = $FileObject.FullName
            }
            $thisObject.xml = $PS1XML.TextDefinition
            $xml += $thisObject
        }
    }
    End{
