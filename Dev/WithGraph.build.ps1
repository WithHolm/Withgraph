...
task GeneratePsXmlCsharp{
    ...
    {
        $XmlFragmentPath = (Join-Path $XmlSaveDir "$($class.fullname).$FragmentEnd")
        Write-output "`t`tLoading class: $($class.fullname)"
        $UsingClass = new-object $class.fullname

        Write-Output "`t`tSearching for hidden properties"
